 ...
 */
#define MAX_FNAME_LENGTH 55   /* Assume at most 20 characters (16.3) */         // CHANGED THIS
...
main(int argc, char **argv)
{
  ...
  if (fds[0] >= 0) {
      int filesize0 = sfs_getfilesize(names[0]);        // CHANGED THIS
      sfs_fseek(fds[0], filesize0);                 // CHANGED THIS
    for (i = 0; i < 100000; i++) {
      int x;
  ...
  char *filename = (char *)malloc(MAX_FNAME_LENGTH);        // CHANGED THIS
  int max = 0;
  while (sfs_getnextfilename(filename) == 0) {             // CHANGED THIS
  ...
  if (sfs_getnextfilename(filename) == 0) {             // CHANGED THIS
